...
if [[ ! -f "$edge_wasm" ]]; then
  printf 'error: the tested Linux job guest is missing: %s\n' "$edge_wasm" >&2
  exit 1
fi
...
"$script_dir/assemble-bundle.sh" "$source_root" darwin-arm64 "$edge_wasm"
"$script_dir/verify-bundle.sh" "$root/out/wurster-edge-runtime-darwin-arm64"
